import { Controller, Get, Post, Query, Body, UseGuards, Request, Res, Logger } from '@nestjs/common';
...
export class ShopifyBillingController {
  ...
  @UseGuards(ShopifyAuthGuard)
  @Post('subscribe')
  async subscribe(@Body() body: { plan: 'BASIC' | 'PRO'; mock?: boolean }, @Request() req: any) {
  ...
  async callback(
    ...
    try {
      ...
      const apiKey = this.config.get<string>('SHOPIFY_API_KEY') || 'shopbox-dev-1';
